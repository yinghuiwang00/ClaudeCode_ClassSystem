...
class UserControllerTest_new {
    ...
    void shouldReturn401WhenGettingCurrentUserWithoutAuthentication() throws Exception {
        // When & Then
        mockMvc.perform(get("/api/v1/users/me"))
    ...
    @DisplayName("Should return 403 when non-admin tries to get user by ID")
    ...
    @Test
    @DisplayName("Should return 401 when getting user by ID without authentication")
    void shouldReturn401WhenGettingUserByIdWithoutAuthentication() throws Exception {
        // When & Then
        mockMvc.perform(get("/api/v1/users/1"))
            .andExpect(status().isUnauthorized());
    ...
    @Test
    @DisplayName("Should return 401 when getting all users without authentication")
    void shouldReturn401WhenGettingAllUsersWithoutAuthentication() throws Exception {
        // When & Then
        mockMvc.perform(get("/api/v1/users"))
            .andExpect(status().isUnauthorized());
